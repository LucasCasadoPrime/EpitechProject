...
# Evaluate the model with a different threshold
threshold = 0.3
...
print(f"Train F1 Score: {f1_score(y_train, rf.predict(X_train))}")
print(f"Test F1 Score: {f1_score(y_test, y_test_pred)}\n")

print(f"Train Recall: {recall_score(y_train, rf.predict(X_train))}")
print(f"Test Recall: {recall_score(y_test, y_test_pred)}")

print(f"Train Precision: {precision_score(y_train, rf.predict(X_train))}")
...
# Pie charts for Random Forest and KNN
labels_pie = ['True Negatives', 'False Positives', 'False Negatives', 'True Positives']
rf_values = [tn, fp, fn, tp]

fig, ax = plt.subplots()
ax.pie(rf_values, labels=labels_pie, autopct='%1.1f%%', startangle=140)
ax.set_title('Random Forest Error Distribution')
plt.show()

# Histogram for False Positives and False Negatives
labels_fp_fn = ['False Positives', 'False Negatives']
rf_fp_fn_values = [fp, fn]

fig, ax = plt.subplots()
ax.bar(labels_fp_fn, rf_fp_fn_values, color=['red', 'blue'])
ax.set_title('Random Forest False Positives and False Negatives')
plt.show()
