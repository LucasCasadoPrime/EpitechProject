...
int my_is_sep2(const char c, const char *separator)
{
    int is_sep = 0;
...
int get_nb_words2(char *str, const char *separator)
{
    int is_sep = 0;
    int nb_words = 0;
    int nb_chars = 0;
    ...
    for (int i = 0; str[i] != '\0'; i++) {
        is_sep = my_is_sep2(str[i], separator);
        if (is_sep == 0)
            nb_chars += 1;
...
{
    char **array = malloc(sizeof(char *) * (get_nb_words2(str, separator) + 1));
    int is_sep = 0;
    int row = 0;
    int nb_chars = 0;
    int to = 0;
    ...
    for (int i = 0; str[i] != '\0'; i++) {
        ...
        if ((is_sep == 1 && nb_chars > 0) || (is_sep == 0 && str[i+1] == 0)) {
            ...
            if (is_sep == 0)
                to = 1;
            array[row] = my_strncpy2(array[row], &str[i-nb_chars+to], nb_chars);
            row++;
            nb_chars = 0;
        }
    }
    array[get_nb_words2(str, separator)] = 0;
    return (array);
}